...
import type {
  TaskFiltersModel,
  TaskResponse,
} from "@/shared/model/store/slices/task/task.model";
...
export const taskApi = createApi({
  ...
  endpoints: (builder) => ({
    getTasks: builder.query<
      TaskResponse,
      Partial<Pagination & TaskFiltersModel>
    >({
    ...
    updateTask: builder.mutation<
      void,
      { data: Partial<TaskSchemaType>; id: Pick<Task, "id"> }
    >({
      query: ({
        data,
        id,
      }: {
        data: Partial<TaskSchemaType>;
        id: Pick<Task, "id">;
      }) => ({
        url: `/api/task/${id.id}`,
        method: "PATCH",
