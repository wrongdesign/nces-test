  ...
  ({
    ...
    return (
      <Button
        ...
        variant={
          expiredDeadline && status !== TaskStatusEnum.DONE
            ? "destructive"
            : "secondary"
        }
        onClick={() => {
      ...
      >
        <div className="flex flex-row w-full items-center justify-between mb-3">
          <StatusChange
            status={status}
            updateStatus={async (status: TaskStatusType) => {
        ...
        <h3 className="font-bold text-gray-900 dark:text-white mb-2 text-[1.25rem] text-left">
          {title}
        </h3>

        <p className="max-w-[90%] whitespace-pre-wrap text-gray-600 dark:text-white">
          {description}
        </p>
        ...
        <div className="flex flex-row gap-2 flex-wrap">
          {tags &&
            ...
            tags.map((tag) => (
              ...
              >
                {tagsList?.find((tagObject) => tagObject.id === tag)?.name ??
                  tag}
              </Badge>
            ))}
        </div>
