...
const SelectFiltersList = ({ tags }: Props) => {
  ...
  const selectedStatus = useMemo(
    () => (filters?.status ? StatusesLabeled[filters.status].label : undefined),
    [filters?.status],
  );
  const selectedPriority = useMemo(
    () =>
      filters?.priority ? PriorityLabeled[filters.priority].label : undefined,
    [filters?.priority],
  );
      ...
      <CustomSelect
        ...
        onValueChange={(value) =>
          dispatch(
            setFilters({
              ...filters,
              priority: value === "all" ? undefined : (value as PriorityType),
            }),
          )
        }
      >
        {PriorityArray.map((item) => {
          return (
            <SelectItem
              value={item}
              key={`${item}-search-priority-select`}
              className={selectItemClassNames}
            >
          ...
          dispatch(
            setFilters({
              ...
              tag:
                value === "all"
                  ? undefined
                  : tags?.find((item) => item.name === value)?.id,
            }),
          )
        }
      >
