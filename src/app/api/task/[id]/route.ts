...
export async function PATCH(
  ...
  try {
    ...
    const newTask: Task = {
      ...findTask,
      ...body,
      updatedAt: formatDate(date, "sv-SE", "fullString"),
    };
    const filteredTasks: Task[] = mockTasks.filter(
...
export async function DELETE(
  ...
  try {
    ...
    writeFile(
      "src/app/api/task/mocks/tasks.json",
      mockTasks.filter((task: Task) => task.id !== id),
    );
